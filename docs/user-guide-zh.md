...
<p align="center">5 秒清掉一封邮件。本地运行,不上传任何东西到云。</p>

---

## 这是什么 · 5 句话

`imail` 是一个**本地**邮件助手 — 它在你自己电脑上跑,不是 SaaS。

打开它,它会:

1. 用你已有的 Gmail / 163 / Outlook 账号登录
2. 拉取你未读邮件
3. 让 DeepSeek 给每封邮件**一次性写 3 种语气的回复**(positive / neutral / negative)
4. 你按 `1` / `2` / `3` 选一个,自动存到你邮箱原本的"草稿"里
5. 你打开邮箱客户端确认 → 点发送

省下的是**斟酌语气的时间**,不是写邮件本身 — 真正的"yes / no / 让我想想"决策只用 1 秒,语气也只用 1 个按键,不再各种"嗯让我想想这样写会不会太冷"。
