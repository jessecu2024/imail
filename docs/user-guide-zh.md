...
<h1 align="center">imail 上手指南</h1>

<p align="center"><b>邮件再多 — 3 个键搞定。</b></p>
...
"答应不答应"、"约不约"、"什么时候之前回复" — 这些**决策**,你在打开邮件的瞬间就有答案了。

但你**写出来**那一封邮件,花了 30 秒。

剩下的 29 秒,你在做什么?

> 嗯…开头是不是要寒暄一下?<br>
> "Could you please" 是不是太客气?直接说"Please"会不会太冲?<br>
> 拒绝得加点缓冲句吧,不然显得不专业…<br>
> 等等,签名是 "Best" 还是 "Best regards"?

每一封邮件,你都在重新斟酌一遍这套语气。**收件箱里 30 封未读 = 30 次重新斟酌 = 你下午一整段时间没了。**
...
1. 从你已有的 Gmail / 163 / Outlook 账号拉一封未读邮件
2. 一次性给你 **3 种语气的回复**:**肯定 / 中性 / 拒绝**
3. 你看一眼,按 `1` / `2` / `3` 选一个
4. 自动存进你邮箱原本的**草稿**,你打开手机或网页确认后自己点发送

**1 次决策 + 1 个按键 = 1 封邮件搞定。**

收件箱 30 封未读?**5 分钟全部「已存草稿」**,剩下的时间你拿去做点真正重要的事。

> 🔒 **完全本地运行。** imail 只在你自己电脑上跑,不是 SaaS,不存你的邮件,
> 服务器只监听 `127.0.0.1`,别人访问不到。邮件正文会发到 DeepSeek 生成回复
> (像调任何 AI API 一样),不希望也可以接本地 Ollama —— 见下面 FAQ。
